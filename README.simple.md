...
> 원본 출처와 변경 고지는 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)에 정리했다.
...
Engram Codex는 이 문제를 해결하는 용도로 만들어졌음.

---

## 왜 이름이 Engram임?

`engram`은 경험이 기억으로 남은 흔적을 뜻하는 신경과학 용어다. 이 서버도 대화 중의 사실, 결정, 에러 패턴을 파편 단위로 남겼다가 다음 세션에 다시 꺼내 쓰게 해주니까 이름을 Engram으로 잡았다.
...
실무에서 Codex를 쓰면서 매일 같은 맥락을 반복 설명하는 게 비효율적이라 느꼈다. 시스템 프롬프트에 메모를 넣는 방법도 써봤지만 한계가 명확했음. 파편 수가 늘어나면 관리가 안 되고, 검색이 안 되고, 오래된 정보와 새 정보가 충돌함.
...
MCP 서버가 AI에게 기억 도구 사용을 권장하지만, 세션 시작 시 자동으로 과거 기억을 불러오지는 않는다. `AGENTS.md`에 아래 지시를 추가하면 Codex가 스스로 context를 로드한다:

```markdown
## 세션 시작 규칙
- 대화 시작 시 `context` 도구를 호출하여 Core Memory를 로드한다.
- 에러/코드 작업 전 `recall(keywords=[관련_키워드])`로 관련 기억을 먼저 확인한다.
- 작업 마일스톤이나 종료 전 `reflect`를 호출한다.
```

Codex app과 CLI는 MCP 설정을 공유하므로 `~/.codex/config.toml` 한 곳에서 서버를 연결하고, 저장소별 규칙은 `AGENTS.md`에 두는 편이 가장 관리하기 쉽다.
...
1. **비용 절약.** 다른 서비스(Mem0 등)는 기억을 찾을 때마다 매번 OpenAI 임베딩 API를 호출함. Engram Codex는 키워드로 바로 찾을 수 있으면 API 호출 없이 끝냄. 기억이 수백 개 쌓여도 API 비용이 비례해서 늘지 않고 더 효율적.
...
맞음. Engram Codex도 `remember` 시 해시 충돌이면 merge함 (`merged: true` 반환). 거기에 더해서 지금은 `remember` 시점에 pgvector로 같은 주제 유사 파편을 직접 찾아서 `related`, `resolved_by`, `superseded_by` 같은 관계를 자동으로 걸어줌. 중복 정리는 consolidate에서 배치로 처리하고, 관계 생성은 저장 시점에 바로 하는 하이브리드 방식. Mem0가 저장할 때마다 실시간으로 전부 임베딩 비교하는 거랑 다르게, Engram Codex는 저장은 빠르게 하되 유사도 기반 관계만 즉시 걸고 무거운 정리는 나중에 하는 구조.
