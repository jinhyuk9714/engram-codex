...
Engram Codex는 MCP(Model Context Protocol) 기반 에이전트 장기 기억 서버다. Codex Desktop과 CLI에서 같은 기억 흐름을 재사용할 수 있도록 설계됐으며, 세션이 종료되어도 중요한 사실, 결정, 에러 패턴, 절차를 유지하고 다음 세션에서 복원한다.
...
| `onboarding` | 시스템 사용법 안내 | AI가 Engram Codex 도구를 언제 어떻게 써야 하는지 스스로 학습 |
...
설치, 마이그레이션, Codex app/CLI 연결은 **[INSTALL.md](INSTALL.md)**를 참조한다.
