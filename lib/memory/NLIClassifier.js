 ...
 *   - 별도 Docker 컨테이너 기반 NLI 서비스 등 사전 배포된 서비스 활용
 *   - 모델 로딩/메모리 오버헤드 없음
 *
...
import { NLI_DISABLE_INPROCESS, NLI_SERVICE_URL, NLI_TIMEOUT_MS } from "../config.js";

/** ─── 공유 상태 ─── */
let _mode    = NLI_SERVICE_URL ? "external" : (NLI_DISABLE_INPROCESS ? "disabled" : "inprocess");
let _failed  = false;
...
async function loadModel() {
  if (_mode !== "inprocess" || _isShuttingDown) return null;
...
export function isNLIAvailable() {
  if (_isShuttingDown || _mode === "disabled") return false;
...
export async function classifyNLI(premise, hypothesis) {
  if (_isShuttingDown || _mode === "disabled") return null;
  if (_mode === "external") {
...
export async function preloadNLI() {
  ...
  if (_mode === "disabled") {
    console.log("[NLIClassifier] In-process mode disabled by NLI_DISABLE_INPROCESS=true");
    return;
  }
  ...
  if (_mode === "external") {
    try {
      ...
      if (NLI_DISABLE_INPROCESS) {
        console.warn(`[NLIClassifier] External service health check failed: ${err.message}, in-process fallback disabled`);
        _mode = "disabled";
        return;
      }
...
export async function shutdownNLI() {
  ...
  _shutdownPromise = (async () => {
    try {
      ...
      console.warn(`[NLIClassifier] Shutdown cleanup failed: ${err.message}`);
    } finally {
      _tokenizer = null;
      _model = null;
      _id2label = null;
...
export function __setNLIStateForTests(state = {}) {
  if ("mode" in state) _mode = state.mode;
  if ("failed" in state) _failed = state.failed;
  if ("tokenizer" in state) _tokenizer = state.tokenizer;
  if ("model" in state) _model = state.model;
  if ("id2label" in state) _id2label = state.id2label;
  if ("loading" in state) _loading = state.loading;
  if ("isShuttingDown" in state) _isShuttingDown = state.isShuttingDown;
  if ("shutdownPromise" in state) _shutdownPromise = state.shutdownPromise;
}
