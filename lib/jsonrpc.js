...
const STATIC_LIST_PAGE_SIZE = 50;
const ADVERTISED_MCP_CAPABILITIES = "tools, prompts, resources";
...
/**
 * 정적 배열용 cursor 기반 페이지네이션
 *
 * @param {Array} items
 * @param {Object} params
 * @returns {{ itemsSlice: Array, nextCursor?: string }}
 */
export function paginateStaticItems(items, params = {}) {
  let offset = 0;
  ...
  const itemsSlice = items.slice(offset, offset + STATIC_LIST_PAGE_SIZE);
  const hasMore = offset + STATIC_LIST_PAGE_SIZE < items.length;
  ...
  return {
    ...
    nextCursor: Buffer.from(
      JSON.stringify({ offset: offset + STATIC_LIST_PAGE_SIZE })
    ).toString("base64url")
  };
}
...
export async function handleInitialize(params) {
  ...
  try {
    ...
    const aiInstructions     = `# Engram Codex Server

연결 성공. Codex를 위한 Fragment-Based Memory 시스템.

Advertised MCP capabilities: ${ADVERTISED_MCP_CAPABILITIES}
...
2. 저장소에 지속 지침이 없다면 AGENTS.md 또는 Codex 프로젝트 지침에 다음 규칙을 남기는 것을 우선 제안한다.
   - 세션 시작 시 context 호출
   - 에러 해결이나 코드 변경 전 recall 우선 확인
   - 작업 마일스톤이나 종료 전 reflect 수행
   - 중요한 사실/결정/절차 확정 시 remember 저장

3. 저장소에 이미 AGENTS.md나 Codex 프로젝트 지침이 있다면 그 규칙을 우선 따르고, 중복 규칙을 다시 만들지 않는다.
...
지원 버전: ${SUPPORTED_PROTOCOL_VERSIONS.join(", ")}`;
    ...
    const result = {
      ...
      serverInfo     : {
        name       : "engram-codex-server",
        version    : "1.0.0",
        description: `Engram Codex - Fragment-Based Memory Server for Codex (도구 12개)
 ...
 * tools/list 핸들러
 */
export function handleToolsList(params) {
  const startTime        = process.hrtime.bigint();

  try {
    const paged = paginateStaticItems(getToolsDefinition(), params);
    const result = {
      tools: paged.itemsSlice
    };
 ...
 * prompts/list 핸들러
 */
export function handlePromptsList(params) {
  const startTime        = process.hrtime.bigint();

  try {
    const paged = paginateStaticItems(PROMPTS, params);
    const result = {
      prompts: paged.itemsSlice
    };
 ...
 * resources/list 핸들러
 */
export function handleResourcesList(params) {
  const startTime        = process.hrtime.bigint();

  try {
    const paged = paginateStaticItems(RESOURCES, params);
    const result = {
      resources: paged.itemsSlice
    };
