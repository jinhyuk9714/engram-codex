...
export async function validateAuthentication(req, msg) {
  ...
  /** 1. engram-access-key 헤더 체크 */
  const accessKeyHeader    = req.headers["engram-access-key"];

  if (accessKeyHeader && safeCompare(accessKeyHeader, ACCESS_KEY)) {
    return { valid: true };
  }
